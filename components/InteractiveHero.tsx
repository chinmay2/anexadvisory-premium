...
export default function InteractiveHero() {
  ...
  return (
    <section className="reference-hero" onPointerMove={move} onPointerLeave={leave} aria-labelledby="hero-title">
      ...
      <style jsx>{`
        .reference-hero{position:relative;min-height:780px;height:min(900px,100vh);overflow:hidden;isolation:isolate;background:#050a0d;color:#f8f5ef}
        ...
        /* Warm, muted architectural treatment to match the supplied reference. */
        .reference-hero__image{z-index:-5;background-repeat:no-repeat;background-size:auto 94%;background-position:100% 50%;background-color:#050a0d;filter:saturate(.46) contrast(1.06) brightness(.56) sepia(.12);transform:scale(.94);transform-origin:right center;transition:transform 1.2s cubic-bezier(.2,.7,.2,1),filter 1s ease}
        .reference-hero:hover .reference-hero__image{transform:scale(.955);filter:saturate(.52) contrast(1.07) brightness(.59) sepia(.14)}
        .reference-hero__shade{z-index:-4;background:linear-gradient(90deg,rgba(3,8,11,1) 0%,rgba(3,8,11,.985) 16%,rgba(3,8,11,.91) 30%,rgba(3,8,11,.68) 45%,rgba(3,8,11,.25) 64%,rgba(3,8,11,.18) 100%),linear-gradient(0deg,rgba(3,8,11,.9),rgba(3,8,11,.08) 43%,rgba(3,8,11,.34))}
        /* Remove the strong electric-blue cast; use restrained bronze/amber illumination instead. */
        .reference-hero__blueGlow{z-index:-3;background:radial-gradient(circle at 78% 47%,rgba(224,171,91,.16),transparent 27%),radial-gradient(circle at 93% 30%,rgba(82,88,96,.11),transparent 29%),radial-gradient(circle at 69% 76%,rgba(195,139,59,.10),transparent 25%);mix-blend-mode:screen}
        .reference-hero__grain{z-index:-2;opacity:.035;background-image:radial-gradient(rgba(255,255,255,.8) .5px,transparent .7px);background-size:5px 5px;mix-blend-mode:soft-light}
