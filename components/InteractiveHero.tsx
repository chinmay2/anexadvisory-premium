  ...
  return (
    <section className="reference-hero" onPointerMove={move} onPointerLeave={leave} aria-labelledby="hero-title">
      ...
      <style jsx>{`
        ...
        /* Zoom OUT: show more of the source architecture instead of using cover crop. */
        .reference-hero__image{z-index:-5;background-repeat:no-repeat;background-size:auto 94%;background-position:100% 50%;background-color:#030a0f;filter:saturate(.64) contrast(1.08) brightness(.58);transform:scale(.94);transform-origin:right center;transition:transform 1.2s cubic-bezier(.2,.7,.2,1),filter 1s ease}
        .reference-hero:hover .reference-hero__image{transform:scale(.955);filter:saturate(.7) contrast(1.08) brightness(.61)}
        .reference-hero__shade{z-index:-4;background:linear-gradient(90deg,rgba(2,8,12,1) 0%,rgba(2,8,12,.99) 17%,rgba(2,8,12,.9) 31%,rgba(2,8,12,.62) 46%,rgba(2,8,12,.2) 67%,rgba(2,8,12,.34) 100%),linear-gradient(0deg,rgba(2,8,12,.94),rgba(2,8,12,.12) 43%,rgba(2,8,12,.36))}
        .reference-hero__blueGlow{z-index:-3;background:radial-gradient(circle at 91% 34%,rgba(36,72,255,.46),transparent 29%),radial-gradient(circle at 76% 70%,rgba(214,161,78,.14),transparent 26%);mix-blend-mode:screen}
